...
// REVISI CHECKOUT: Hapus WhatsApp, Ganti dengan Notifikasi Sukses, dan Kosongkan Keranjang
checkoutBtn?.addEventListener('click', () => {
    ...
    const totalPrice = cart.reduce((sum, it) => sum + it.price * it.quantity, 0);
    const text = `Pesanan Anda: ${cart.map(i => i.name + ' x' + i.quantity).join(', ')}. Total: Rp ${totalPrice.toLocaleString('id-ID')}`;

    // 1. Notifikasi berhasil
    alert('Checkout Berhasil! ' + text + '. Pesanan Anda telah diproses.');

    // 2. Kosongkan keranjang
    cart = [];

    // 3. Perbarui UI dan tutup keranjang
    closeCart();
    updateCartCount();
    renderCart();
});
